    ...
    module Authorizable
      ...
      module ClassMethods
        ...
        def acts_as_authorizable(options = {})
          ...
          class_attribute :acts_as_authorizable_sources
          class_attribute :acts_as_authorizable_options

          self.acts_as_authorizable_sources= []
          self.acts_as_authorizable_options= conf
      ...
      module AuthSingletonMethods
        ...
        def auth_belongs_to_user(assoc,options = {})
          ...
          self.acts_as_authorizable_sources << conf
        end
      ...
      module InstanceMethods
        ...
        def authorized?(user,permission,excludes=[])
          ...
          self.acts_as_authorizable_sources.each do |source|
            case source[:type]
        ...
        def auth_check_sources!
          raise 'No authorizable sources' if self.acts_as_authorizable_sources.empty?
        end
        ...
        def auth_assoc_using_has_many_parents(source, user)
          ...
          if source[:user_scope].nil?
            ...
            parents = self.send(assoc).send(source[:user_scope], user)
          end
        ...
        def auth_locate_role_object(role)
          self.acts_as_authorizable_options[:role_class_name].constantize.send(acts_as_authorizable_options[:role_locate_method],role)
        end
      end
    end
  end
